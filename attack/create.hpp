...
template<typename T>
void get_random_set(
                    std::set<T> &random_set, // the set containing the random numbers
...
typedef std::function<void(uint32_t, std::set<uint64_t>&)> obtain_evict_set_func_t;

extern bool
targeted_evict_set_creator(
                           uint32_t num,                  // number of lines to be generated
                           std::set<uint64_t>& candidate, // the generated candidates
