...
int main(int argc, char* argv[]) {
  if(argc < 13) {
    ...
    std::cout << std::endl;
    std::cout << "test_eviction nl1 nl1s nl1w nllc nllcs nllcw level size cycle write threshold N [report]" << std::endl;
    return 0;
  }
  ...
  for(uint32_t t=0; t<testN; t++) {
    ...
    m_way = cache_level == 1 ? NL1Way : NLLCWay;
    ...
    if(!targeted_trim_original((L1CacheBase *)l1_caches[0], target, candidate, ProbeCycle, ProbeWrite, ProbeThreshold, hit, check, m_way))
      continue;
