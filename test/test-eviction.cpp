...
int main(int argc, char* argv[]) {
  if(argc < 7) {
    ...
    std::cout << std::endl;
    std::cout << "test_eviction level size cycle write threshold N [file]" << std::endl;
    return 0;
  }
  ...
  stat_mean  l1_evict_stat, l1_full_stat;
  stat_mean  llc_evict_stat, llc_full_stat;
  ...
  for(uint32_t t=0; t<testN; t++) {
    ...
    double l1_creation_access = (double)REPORT::get_l1_accessed(0);
    double llc_creation_access = (double)REPORT::get_llc_accessed();
    REPORT::clear();

    if(!targeted_trim_original(l1_caches[0], target, candidate, hit))
      continue;

    double l1_evict_access = (double)REPORT::get_l1_accessed(0);
    double llc_evict_access = (double)REPORT::get_llc_accessed();

    l1_evict_stat(l1_evict_access);
    llc_evict_stat(llc_evict_access);
    l1_full_stat(l1_evict_access + l1_creation_access);
    llc_full_stat(llc_evict_access + llc_creation_access);
  }

  std::cout << "sucess " << count(l1_full_stat) << " time in " << testN << " trials." << std::endl;
  std::cout << "L1 cache access: " << mean(l1_full_stat) << "+/-" << error_of<tag::mean>(l1_full_stat)<< std::endl;
  std::cout << "LLC cache access: " << mean(llc_full_stat) << "+/-" << error_of<tag::mean>(llc_full_stat)<< std::endl;

  if(fn != "") {
    std::ofstream outfile(fn, std::ofstream::app);
    outfile << candidate_size << "\t";
    outfile << testN << "\t";
    outfile << count(l1_full_stat) << "\t";
    outfile << (cache_level==1 ?  mean(l1_full_stat) : mean(llc_full_stat)) << "\t";
    outfile << (cache_level==1 ?  error_of<tag::mean>(l1_full_stat) : error_of<tag::mean>(llc_full_stat)) << "\t";
    outfile << (cache_level==1 ?  mean(l1_evict_stat) : mean(llc_evict_stat)) << "\t";
    outfile << (cache_level==1 ?  error_of<tag::mean>(l1_evict_stat) : error_of<tag::mean>(llc_evict_stat)) << std::endl;
  }
  return 0;
}
